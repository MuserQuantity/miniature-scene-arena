...
function FilterSelect({ value, options, label, change }: { value: string; options: { label: string; value: string }[]; label: string; change: (value: string) => void }) {
  return <Select value={value} onValueChange={(next) => next && change(next)} items={options}>
...
function SceneCard({ scene, compact, returnTo }: { scene: SceneRecord; compact: boolean; returnTo: string }) {
  ...
  return <article className={cn('gallery-feature grid lg:grid-cols-[minmax(0,1fr)_21rem]', compact && 'lg:grid-cols-[20rem_minmax(0,1fr)]')}>
    <Link href={href} className={cn('feature-image min-h-72 lg:min-h-full', compact ? 'aspect-[4/3]' : 'aspect-[3/2] lg:aspect-auto')} aria-label={`进入作品：${scene.title}`}>
      <Image src={scene.cover} alt="雨夜便利店：冷蓝色街角上的暖光店铺、湿润街道、自行车与自动贩卖机，真实三维场景截图" width={1440} height={1000} priority sizes="(max-width: 1024px) 100vw, 850px" className="h-full w-full object-cover" />
      <div className="absolute top-5 left-5"><Badge variant="secondary"><Box data-icon="inline-start" />可交互 3D 场景</Badge></div>
      <div className="absolute bottom-5 left-5 flex items-center gap-2 text-sm text-foreground/70"><span className="size-1.5 rounded-full bg-primary" />实际场景渲染</div>
    ...
    <div className={cn('flex flex-col justify-between border-t lg:border-t-0 lg:border-l', compact && 'lg:min-h-80')}>
      <div className="p-7 lg:p-8">
        <div className="flex flex-col gap-5">
          <div className="flex items-center justify-between gap-3"><span className="eyebrow">精选收录</span><span className="font-mono text-sm text-muted-foreground">No. 001</span></div>
      ...
      <div className="px-7 pb-7 lg:px-8 lg:pb-8">
        <div className="flex flex-col gap-5">
          ...
          <dl className="flex flex-col gap-3">
            ...
            <div className="metadata-row"><dt>创作模型</dt><dd className="text-muted-foreground">{scene.model}</dd></div>
          </dl>
          <Link className={cn(buttonVariants(), 'h-11 w-full')} href={href}>走进这个世界 <ArrowRight data-icon="inline-end" /></Link>
          <p className="text-center text-sm text-muted-foreground">自由旋转 · 放大细节 · 沉浸观看</p>
        </div>
      </div>
    </div>
  </article>
...
export function GalleryBrowser({ scenes }: { scenes: readonly SceneRecord[] }) {
  ...
  function update(key: string, value: string) {
    ...
    const defaults: Record<string, string> = { model: 'all', agent: 'all', tag: 'all', layout: 'grid', sort: 'newest', page: '1' }
  ...
  const filtered = scenes.filter((scene) => {
    ...
    return haystack.includes(query.trim().toLowerCase()) && (model === 'all' || scene.model === model) && (agent === 'all' || scene.agent === agent) && (tag === 'all' || scene.category === tag)
  ...
  const modelOptions = [{ value: 'all', label: '全部模型' }, ...Array.from(new Set(scenes.map((scene) => scene.model))).map((value) => ({ value, label: value }))]
  const agentOptions = [{ value: 'all', label: '全部 Agent' }, ...Array.from(new Set(scenes.map((scene) => scene.agent).filter(Boolean))).map((value) => ({ value, label: value }))]
  const isFiltered = query || model !== 'all' || agent !== 'all' || tag !== 'all'
  return <section id="collection" className="flex flex-col gap-6 reveal-late" aria-label="场景作品集">
    <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
      <div className="flex items-center gap-4">
        <ToggleGroup value={[tag]} onValueChange={(value) => value.length && update('tag', value[0] as string)} aria-label="作品分类" spacing={3}>
          <ToggleGroupItem value="all">全部场景 <span className="font-mono text-muted-foreground">{scenes.length}</span></ToggleGroupItem>
          <ToggleGroupItem value="建筑与空间">建筑与空间</ToggleGroupItem>
        </ToggleGroup>
      </div>
